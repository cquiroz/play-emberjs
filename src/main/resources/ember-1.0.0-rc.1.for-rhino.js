  ...
  function() {
    ...
    StaticSegment.prototype = {
      eachChar: function(callback) {
        var string = this.string, c;

        for (var i=0, l=string.length; i<l; i++) {
          c = string.charAt(i);
          callback({ validChars: c });
        }
      },
      ...
      match: function(c) {
        ...
        for (var i=0, l=nextStates.length; i<l; i++) {
          ...
          if (typeof (chars = charSpec.validChars) !== 'undefined') {
            if (chars.indexOf(c) !== -1) { returned.push(child); }
    ...
    function recognizeChar(states, c) {
      var nextStates = [];

      for (var i=0, l=states.length; i<l; i++) {
        var state = states[i];

        nextStates = nextStates.concat(state.match(c));
      }
    ...
    function addSegment(currentState, segment) {
      segment.eachChar(function(c) {
        var state;

        currentState = currentState.put(c);
      });
